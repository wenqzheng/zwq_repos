...
int main ()
{
	...
	valueType pi;
	if (i == 1)
        pi = Strategy(s1);//std::make_shared<Strategy>(s1);
	else if (i == 2)
	    pi = Strategy(s2);//std::make_shared<Strategy>(s2);
	else if (i == 3)
     	pi = Strategy(s3);//std::make_shared<Strategy>(s3);
	else {
	    cout << "error num" << endl;
	    return;
	}
